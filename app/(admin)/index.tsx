    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      >
        ...
        <View style={styles.header}>
          <Text style={styles.pageTitle}>{locale === 'es' ? 'Panel de Administración' : 'Admin Panel'}</Text>
          <Text style={styles.pageSubtitle}>{locale === 'es' ? 'Resumen general y métricas clave del sistema' : 'System overview and key performance metrics'}</Text>
        </View>
        ...
        {loading || !stats ? (
          ...
          <>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>{locale === 'es' ? 'Contratos Recientes' : 'Recent Leases'}</Text>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.actionsGrid}>
                {[
                  { key: 'users', label: locale === 'es' ? 'Gestionar Usuarios' : 'Users Admin', icon: 'account-cog' as const, color: accentColor, route: '/(admin)/users' },
                  { key: 'listings', label: locale === 'es' ? 'Gestionar Alojamientos' : 'Listings Admin', icon: 'home-cog' as const, color: '#3b82f6', route: '/(admin)/listings' },
                  { key: 'contracts', label: locale === 'es' ? 'Moderar Contratos' : 'Contracts Hub', icon: 'file-cog' as const, color: '#49c788', route: '/(admin)/contracts' },
                  { key: 'reports', label: locale === 'es' ? 'Reportes de Seguridad' : 'Reports Hub', icon: 'alert-cog' as const, color: '#f97316', route: '/(admin)/reports' },
                  { key: 'verifications', label: locale === 'es' ? 'Verificación de Identidad' : 'Verifications', icon: 'shield-cog' as const, color: '#06b6d4', route: '/(admin)/verifications' },
                  { key: 'settings', label: locale === 'es' ? 'Ajustes del Sistema' : 'System Settings', icon: 'cog-outline' as const, color: '#888', route: '/(admin)/settings' },
                ].map((a) => (
                  <TouchableOpacity
                    key={a.key}
