...
// Pantalla de Política de Privacidad, renderiza secciones traducidas desde el diccionario de idiomas
export default function PrivacyPolicyScreen() {
  const { t, locale } = useTranslation();

  const icons = ['📋', '🎯', '🛡️', '📍', '💳', '🔗', '🔒', '⚖️', '👤', '🔄'];
  const sections = (translations[locale]?.privacy_policy?.sections || []).map((sec: any, idx: number) => ({
    ...sec,
    icon: icons[idx] || '📄'
  }));
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <LinearGradient colors={['#0d1117', '#000']} style={s.header}>
        ...
        <Text style={s.headerTitle}>{t('privacy_policy.title')}</Text>
        <View style={{ width: 36 }} />
      </LinearGradient>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.hero}>
          <View style={s.heroIconWrap}>
            <MaterialCommunityIcons name="shield-lock" size={40} color="#49C788" />
          </View>
          <Text style={s.heroTitle}>{t('privacy_policy.hero_title')}</Text>
          <Text style={s.heroSub}>
            {t('privacy_policy.hero_sub')}
          </Text>
        </View>

        {/* Nota sobre datos de verificación */}
        <View style={s.intermediaryCard}>
          ...
          <Text style={s.intermediaryText}>
            <Text style={{ fontWeight: '800', color: '#49C788' }}>{t('privacy_policy.intermediary_title')}</Text>
            {' '}{t('privacy_policy.intermediary_desc')}
          </Text>
        </View>
        ...
        <View style={s.footer}>
          <Text style={s.footerText}>
            {t('privacy_policy.questions_note')}
          </Text>
          <Text style={s.footerVersion}>{t('privacy_policy.footer_version')}</Text>
        </View>
