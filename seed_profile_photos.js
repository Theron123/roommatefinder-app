...
const MALE_NAMES = ["carlos", "david", "alejandro", "mateo", "lucas", "thiago", "benjamin", "santiago", "joaquin", "diego", "sebastian"];
const FEMALE_NAMES = ["sofia", "valeria", "valentina", "camila", "isabella", "martina", "emma", "lucia", "victoria"];

function getThemeForUser(name, index) {
  const normName = name.toLowerCase();
  const isFemale = FEMALE_NAMES.some(f => normName.includes(f));
  const isMale = MALE_NAMES.some(m => normName.includes(m));
...
async function seed() {
  ...
  for (let i = 0; i < profiles.length; i++) {
    ...
    // Update photoUrl to match the first photo in the cohesive set,
    // and store the array of 5 matching photos in the 'photos' column.
    ...
    if (updateErr) {
      ...
      console.log(`Updated ${name} with 5 cohesive Unsplash photos.`);
      count++;
    }
  }

  console.log(`Done! Successfully updated ${count} profiles with 5 matching photos each.`);
}
