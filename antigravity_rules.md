...
- **Secretos de Supabase Edge Functions** (no viven en ningún archivo del repo — se configuran una vez con `supabase secrets set NOMBRE=valor --project-ref jwzcvozwygsfkouclhrz` y quedan guardados del lado de Supabase):
  ...
  - `RESEND_FROM_EMAIL` — dirección remitente. **Configurado (12 de julio, 2026)** a `RoomieMates <noreply@mail.roomiemates.com>`, sobre el dominio `mail.roomiemates.com` ya verificado en Resend (DKIM + SPF/MX en verde vía Cloudflare). Antes de esto usaba el sandbox `onboarding@resend.dev`, que solo entregaba a la cuenta propia del dueño del proyecto — ya no aplica esa restricción, confirmado con un envío real a un destinatario distinto.
...
- **[12 de Julio, 2026] - Trust Center: OTP real por email (Prioridad 2 de deuda técnica):**
  ...
  - **✅ Resuelto (12 de julio, 2026):** dominio `mail.roomiemates.com` verificado en Resend (DKIM, SPF/MX en verde vía Auto configure con Cloudflare; DMARC quedó opcional/sin verificar, no bloquea el envío). Secreto `RESEND_FROM_EMAIL` actualizado a `RoomieMates <noreply@mail.roomiemates.com>`. Confirmado con un envío real de prueba a un destinatario que **no** es la cuenta del dueño del proyecto — ya no aplica la restricción de sandbox. El OTP por email ya funciona para cualquier usuario real, no solo para pruebas.
  - Pendiente menor, no bloqueante: la API key de Resend configurada como secreto sigue siendo de **solo envío** — si en el futuro se necesita gestionar dominios vía API (en vez del dashboard), hay que generar una key con permisos completos.

- **[12 de Julio, 2026] - Auditoría de ciberseguridad completa + remediación de 5 críticos y 3 altos:**
  - Se hizo una auditoría profesional de todo el proyecto (arquitectura, RLS real, Storage, Edge Functions, dependencias, headers). Reporte completo en el historial de la conversación con Claude — no versionado en el repo. Puntaje inicial: 27/100 general, 14/100 listo-para-producción.
  - **Crítico — escalación de privilegios**: `profiles` permitía que cualquier usuario autenticado se auto-asignara `role='admin'`, `trust_score=100` y cualquier `is_*_verified=true` (sin restricción de columna en la policy de UPDATE/INSERT). **Verificado explotable en vivo, luego corregido** con un trigger `BEFORE INSERT OR UPDATE` (`protect_privileged_profile_columns`) que revierte esos campos si el llamante no es admin.
  - **Colaborador en paralelo**: se descubrió que Sebastian (`Sebastian-cyber01`) trabaja en este proyecto y había agregado, el mismo día, su propia migración de bootstrap de super-admin (`assign_super_admin.sql`, cuenta `admin@roommatefinder.com`) con su propio trigger de protección de `role`. Ambos triggers ahora conviven (ver `20260712204651_reconcile_privesc_fix_with_super_admin_bootstrap.sql`) — cualquier IA que siga trabajando en `profiles` debe revisar ambos antes de tocar esa tabla.
  - **Crítico — Storage bucket "Roommate" público sin autenticación**: cualquiera en internet, sin sesión, podía subir/sobrescribir/borrar fotos de cualquier usuario. Corregido: ahora requiere `authenticated` + que el nombre del archivo empiece con el propio `auth.uid()` (o el prefijo `listing-{uid}-` para anuncios), más límite de 5MB y solo tipos de imagen.
  - **Crítico — Storage bucket "contracts" de lectura pública**: exponía PDFs de contratos (dirección, monto, nombres, firmas) a cualquiera con la URL. Bucket ahora privado; `app/contracts/[id].tsx` genera una URL firmada (5 min) bajo demanda en vez de guardar una URL pública permanente en `pdf_url` (ese campo ahora guarda solo el nombre del archivo).
  - **Crítico — XSS persistente en `generate-contract-pdf`**: esta Edge Function (preexistente, no relacionada con el Trust Center) interpolaba el nombre de perfil del usuario sin escapar en un HTML servido públicamente. Se confirmó que **ya no la usa la app** (la generación de contratos es 100% client-side vía `html2pdf.js`/`expo-print` desde hace tiempo) — se eliminó la función en vez de parcharla.
  - **Crítico — IDOR en `contract_participants`**: `INSERT`/`SELECT` sin ninguna restricción permitían que cualquier usuario se auto-agregara como parte de cualquier contrato ajeno. Corregido para exigir que quien inserta sea el `initiator_id` del contrato. **Esto introdujo un bug de recursión infinita** entre las policies de `contracts` y `contract_participants` (cada una consultaba a la otra) — resuelto con una función `SECURITY DEFINER` (`is_contract_initiator`), mismo patrón que `is_admin()`.
  - **Alto — cabeceras de seguridad HTTP**: `vercel.json` no tenía ninguna (CSP no incluido deliberadamente en esta pasada, por riesgo de romper carga de recursos dinámicos sin poder probarlo en staging). Agregadas X-Frame-Options, X-Content-Type-Options, Referrer-Policy, HSTS, Permissions-Policy.
  - **Alto — política de contraseñas débil**: `password_min_length` 6→10. HIBP (`password_hibp_enabled`) requiere plan Pro de Supabase, no se pudo activar en el tier Free actual. "Requerir contraseña actual para cambiarla" no se pudo aplicar vía Management API (sin error, simplemente no tomó el cambio) — pendiente de activar manualmente desde el Dashboard.
  - **Alto — CORS wildcard en Edge Functions**: `Access-Control-Allow-Origin: '*'` → allowlist (`roomiemates.com`, `www.roomiemates.com`, `localhost:8081` para dev). Verificado que un origen no reconocido ya no se refleja en la respuesta.
  - **Descubrimiento de infraestructura importante**: Vercel está conectado al **fork** `Theron123/roommatefinder-app`, no al repo original `geovannypacheco050-bit/roommatefinder-app` donde vive el remote `origin`. Cualquier cambio de frontend/config necesita push a **ambos** remotes para llegar a producción — `git push origin main && git push fork main:main` (fork agregado como remote `fork`). Los cambios de infraestructura de Supabase (migraciones, RLS, Edge Functions) no se ven afectados por esto, van directo por CLI.
  - Todos los fixes anteriores se probaron contra la base de datos real con cuentas reales (no solo revisión de código) antes y después de cada cambio.
