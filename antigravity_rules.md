...
5. **🟢 Regla de Seguridad Crítica (Service Role Key) — CORREGIDO el 7 de julio, 2026:**
   ...
   - Las operaciones reales de escritura del panel admin (suspender usuario, aprobar verificación, borrar listing, cambiar status de contrato) siempre pasaron por el cliente normal (`lib/supabase.ts`), es decir dependen de RLS. Se creó `supabase/admin_rls_policies.sql` con políticas explícitas (`is_admin()` + policies por tabla en `profiles`, `listings`, `contracts`, `verifications`, `user_reports`) para que esas escrituras funcionen de forma segura sin ninguna key privilegiada en el cliente. **Este script debe ejecutarse manualmente en el SQL Editor del dashboard de Supabase** (la IA no tiene ni debe tener la Service Role Key).
   - **⚠️ Pendiente y solo accionable por el humano:** la key expuesta debe **rotarse** en Supabase (Project Settings → API → "Reset service_role key"), ya que quedó en el historial de git (posiblemente en GitHub) independientemente de que el archivo ya no exista en el working tree actual. Rotar la key invalida cualquier copia filtrada.
   - **Patrón correcto para el futuro:** si el panel admin llega a necesitar una operación que RLS no puede expresar (ej. borrar la cuenta `auth.users` de otro usuario, no solo su fila en `profiles`), la solución es una **Supabase Edge Function** que reciba el JWT del admin, verifique el rol server-side, y use la Service Role Key solo como secreto de servidor (`supabase secrets set`) — nunca en código que compile al bundle del cliente. Ver comentario final de `supabase/admin_rls_policies.sql`.
   - La protección de rutas del panel admin (`app/(admin)/_layout.tsx`) sigue siendo solo client-side (verifica `profiles.role === 'admin'` en el front); la seguridad real depende de las políticas RLS de `supabase/admin_rls_policies.sql` estando aplicadas en el dashboard.
